...
struct _asyncmsg_put_header {
	...
	int (*cb)(int err, void* buf, unsigned handle);
                                        /* user callback */
	unsigned put_hdr_flags;				/* oneof ASYNC_PUT_FLAG_* */
};
#define _ASYNCMSG_PUTHDR_KERNEL_ONLY 0x1U  /* meaning only the kernel should write this put header.
								           the kernel should clear this bit when delivered*/
...
struct _asyncmsg_connection_descriptor {
	...
	pthread_mutex_t mu;						/* mutex to protect the data structure and for the condvar */
											/* #### only code in lib/asyncmsg/_asyncmsg_handle.c may lock/unlock #### */
	unsigned reserve;						/* reserve */
	struct _asyncmsg_connection_attr attr;	/* attribute of this connection */
	unsigned reserves[3];					/* reserve */
	__FLEXARY(struct _asyncmsg_put_header, sendq); /* sendq_size elements long. */
};



/* If MsgReceiveveAsync is preempted, it returns an indication by hiding a bit in the number of parts returned. This is used
   by lib/asyncmsg/asyncmsg_get.c to repeat kernel call until all the messages are received. The user calling asyncmsg_get
   does not see this interaction. */
#define ASYNCMSG_GET_PREEMPT_FLAG	0x80000000
#define ASYNCMSG_GET_RESERVED		0x40000000
#define ASYNCMSG_GET_MASK			0x3FFFFFFF
#define ASYNCMSG_GET_ISVALID(ret) 	( (ret)!=-1 )
#define ASYNCMSG_GET_WAS_PREEMPTED(ret) 	(  ASYNCMSG_GET_ISVALID(ret) && ( (ret) & ASYNCMSG_GET_PREEMPT_FLAG ) )
#define ASYNCMSG_GET_PARTS_RETURNED(ret) ( (ret) & ASYNCMSG_GET_MASK )
...
extern int asyncmsg_channel_create(unsigned flags,  mode_t mode, size_t buffer_size, unsigned max_num_buffer, const struct sigevent *ev, int (*recvbuf_callback)(size_t bufsize, unsigned num_bufs, void*bufs[], int flags, void* callback_handle), void* recvbuf_callback_handle);
...
/* __SRCVERSION("asyncmsg.h $Rev: 361891 $"); */
