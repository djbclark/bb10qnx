...
#define SIGEV_MEMORY			8	/* notify, address, value, op */
...
# define SIGEV_SET_TYPE(__e, __new_type)								\
	((__e)->sigev_notify = ((__e)->sigev_notify & ~SIGEV_TYPE_MASK) | (__new_type))
...
# define SIGEV_GET_TYPE(__e)					\
	((__e)->sigev_notify & SIGEV_TYPE_MASK)


/* to make threads use "critical time" as defined by the optional
 * Adaptive Partitioning scheduler, set a sigevent to be critical
 * with:
 */
# define SIGEV_MAKE_CRITICAL(__e)				\
	((__e)->sigev_notify |= SIGEV_FLAG_CRITICAL)
# define SIGEV_CLEAR_CRITICAL(__e)				\
	((__e)->sigev_notify &= ~SIGEV_FLAG_CRITICAL)
...
#ifdef _STD_USING
using _CSTD clock_t; using _CSTD time_t;
#endif /* _STD_USING */
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/deckard/BB10_2_0X/lib/c/public/sys/siginfo.h $ $REV$")
#endif
