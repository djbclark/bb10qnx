...
#endif
#if defined(__EXT_QNX)
#define PTHREAD_RMUTEX_INITIALIZER	{ {0U}, _NTO_SYNC_INITIALIZER }
#endif
#if defined(__EXT_QNX)
#define PTHREAD_MUTEX_WAKEUP_INITIALIZER	{ {_NTO_SYNC_NONRECURSIVE|_NTO_SYNC_WAKEUP}, _NTO_SYNC_INITIALIZER }
#define PTHREAD_RMUTEX_WAKEUP_INITIALIZER	{ {_NTO_SYNC_WAKEUP}, _NTO_SYNC_INITIALIZER }
#endif

#define PTHREAD_COND_INITIALIZER	{ {_NTO_SYNC_COND}, _NTO_SYNC_INITIALIZER }
...
#define _mutex_haslock(__m)	(((__m)->__owner & ~_NTO_SYNC_WAITING) == __tls()->__owner)
#define _mutex_owner(__m)	((__m)->__owner & ~_NTO_SYNC_WAITING)
#define _mutex_init(__m)	{ const pthread_mutex_t mux = PTHREAD_MUTEX_INITIALIZER; *(__m) = mux; }
#define _mutex_isfree(__m)	(((__m)->__owner == _NTO_SYNC_MUTEX_FREE) || ((__m)->__owner == _NTO_SYNC_INITIALIZER))
#endif
...
#define pthread_mutex_lock(__m)		(__mutex_smp_cmpxchg(&(__m)->__owner, 0, __tls()->__owner) ? (pthread_mutex_lock)(__m) : ((__m)->__u.__count++, 0/* EOK */))
#define pthread_mutex_trylock(__m)	(__mutex_smp_cmpxchg(&(__m)->__owner, 0, __tls()->__owner) ? 16 /* EBUSY */ : ((__m)->__u.__count++, 0 /* EOK */))
...
#define pthread_mutex_timedlock_monotonic(__m, __t)	(__mutex_smp_cmpxchg(&(__m)->__owner, 0, __tls()->__owner) ? (pthread_mutex_timedlock_monotonic)((__m), (__t)) : 0 /* EOK */)
#endif
#endif
#if defined(__EXT_QNX)
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/deckard/BB10_2_0X/lib/c/public/pthread.h $ $REV$")
#endif
