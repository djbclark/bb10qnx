...
#define _NETINET_IP_MROUTE_H_INCLUDED
...
struct vif {
	...
	uint32_t  tbf_n_tok;		/* no of tokens in bucket */
	uint32_t  tbf_q_len;		/* length of queue at this vif */
	uint32_t  tbf_max_q_len;	/* max. queue length */

	uint8_t   v_flags;		/* VIFF_ flags defined above */
	uint8_t   v_threshold;		/* min ttl required to forward on vif */
	uint16_t  zero;			/* align structure */
...
__SRCVERSION( "$URL: http://svn.ott.qnx.com/product/branches/deckard/BB10_2_0X/lib/socket/public/netinet/ip_mroute.h $ $Rev: 233581 $" )
