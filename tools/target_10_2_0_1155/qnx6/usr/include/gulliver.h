...
#ifdef __QNXNTO__
struct _debug_process_info;
struct _procfs_map_info;
struct _debug_thread_info;
struct _debug_channel;
struct _debug_irq;
struct _debug_timer;
struct _procfs_threadctl;
...
extern void __swap_sigset(sigset_t * __set);
extern void __swap_sigevent(struct sigevent * __ev);
extern void __swap_siginfo(struct _siginfo * __sinfo);
extern void __swap_timerinfo(struct _timer_info * __tinfo);


void __swap_procfs_info(struct _debug_process_info * __info);
void __swap_procfs_mapinfo(struct _procfs_map_info * __minfo);
void __swap_procfs_status(struct _debug_thread_info * __status);
void __swap_procfs_channel(struct _debug_channel * __chan);
void __swap_procfs_irq(struct _debug_irq * __irq);
void __swap_procfs_timer(struct _debug_timer * __timer);
void __swap_procfs_threadctl(struct _procfs_threadctl* __tctl, int __outgoing);
void __swap_procfs_sysinfo(struct syspage_entry *__ptr);
void __swap_procfs_debuginfo(struct _procfs_debug_info * __mdbg);
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/deckard/BB10_2_0X/lib/c/public/gulliver.h $ $REV$")
#endif
