...
.elif ${OPSYS} == "QNX"
LOWER_OPSYS?=		qnx
LOWER_ARCH!=		${UNAME} -p | sed -e 's/x86/i386/'
MAKEFLAGS+=		LOWER_ARCH=${LOWER_ARCH:Q}
MACHINE_ARCH?=		${LOWER_ARCH}
OS_VERSION:=		${OS_VERSION:C/^V//}
LOWER_VENDOR?=		qnx
LOWER_OPSYS_VERSUFFIX!= ${UNAME} -r
Q_MACHINE_ARCH!=	${UNAME} -m | sed -e 's/x86pc/pc/'
MACHINE_GNU_PLATFORM?=	${LOWER_ARCH}-${Q_MACHINE_ARCH}-nto-qnx${LOWER_OPSYS_VERSUFFIX}
